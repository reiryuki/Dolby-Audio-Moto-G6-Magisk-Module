...
# ui app
if [ "$MOD_UI" != true ]\
&& [ "`grep_prop dolby.blackui $OPTIONALS`" == 1 ]; then
...
&& [ "$PROP" -gt 192 ]; then
  ...
  if [ "`grep_prop dolby.blackui $OPTIONALS`" == 1 ]; then
    cp -rf $MODPATH/system_black_36dB/* $MODPATH/system
  else
...
hide_oat
APPS="$APPS MusicFX MotoDolbyDax3 MotoDolbyV3
      OPSoundTuner DolbyAtmos daxUser LunarisDolby"
hide_app
if [ "`grep_prop dolby.mod $OPTIONALS`" == 0 ]; then
...
if [ "`grep_prop dolby.virtualizer $OPTIONALS`" == 1 ]; then
  ui_print "- Changing all virtualizer_enable value to true"
  ...
  ui_print "- Changing all virtualizer_enable value to false"
